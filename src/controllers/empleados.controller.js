...
const getAllEmpleados = async (req, res) => {
  ...
  const getEmpleadosByPuesto = async(req, res) => {
    try {
        const [total] = await EmpleadosModel.getNumEmpleadosByPuesto(req.params.puesto);
      const pagina =
        (req.params.pagina - 1) * parseInt(process.env.ELEMENTOS_POR_PAGINA);
        const [result] = await EmpleadosModel.getEmpleadosByPuesto(
            req.params.puesto,parseInt(process.env.ELEMENTOS_POR_PAGINA),
            pagina
        );
        res.json(addPaginado(req.params.pagina, total[0].total, result));
      } catch (error) {
  ...
  const updateEstado = async(req,res) =>{
    try{
        const { idEmpleado } = req.params;
        const [result] = await EmpleadosModel.updateEstadoEmpleado(idEmpleado,req.body);
        res.json(result[0]);
    }catch (error) {
        res.json({ fatal: error.message });
      }
  ...
  const updateAlmacen = async(req,res) =>{
    try{
        const { idEmpleado } = req.params;
        const [result] = await EmpleadosModel.updateAlmacenEmpleado(idEmpleado,req.body);
        res.json(result[0]);
    }catch (error) {
        res.json({ fatal: error.message });
      }
